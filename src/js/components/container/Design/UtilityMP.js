...
export var UtilityMP = function(R) {
	...
	return {
		deleteItem: function deleteItem(item, parentItem) {
			return function traverse(value) {
				debugger
				if (value !== null && Array.isArray(value)) {
		...
		saveItem: function saveItem(item, parentItem) {
			return function traverse(value) {
				if (value !== null && Array.isArray(value)) {
					for (var element in value) {
						traverse(value[element])
		...
		fetchProductsByCategoryId: function fetchProductsByCategoryId(categoryId, catalog) {
			var traverse = traverseCatalog("Product", categoryId)
			return traverse(catalog)
		},
		validate: function validate(item, type) {
			switch (type) {
				case 'PRODUCT':
